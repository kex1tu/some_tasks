...
* `дата_и_время_заявки` вводится в формате `ДД.ММ.ГГГГ чч:мм:сс`, где 
  ...
  - `чч` — часов в виде числа от 0 до 23 или от 00 до 23;
  - `мм` — минут в виде числа от 0 до 59 или от 00 до 59;
  - `сс` — секунд в виде числа от 0 до 59 или от 00 до 59;
...
то необходимо добавить запись на покупку только 1 акции (10-4-5=1), а в записях на продажу исправить количество соответственно на 0 (то есть заявки на продажу были выполнены).
...
Поиск должен быть регистронезависимым (например, одинаковыми являются тикеры `SBERP`, `sberp`, `SberP`).

### Пример команды
Заявка на покупку 5 акций Sber за 315.18 у.е.:
```add 07.03.2025 12:00, SBER, buy, 5, 315.18```
...
* `чч` — часов в виде числа от 00 до 23;
* `мм` — минут в виде числа от 00 до 59;
* `сс` — секунд в виде числа от 00 до 59.
...
## Примечание
Если программа будет разбита на модули, то не забудьте указать нужные для сборки программы .cpp файлы в Makefile в переменной SOURCES.
